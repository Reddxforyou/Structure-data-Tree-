...
address successorPrediction(address root, infotype name[MAX_NAME_LENGTH]) {
    ...
    if (current == NULL) {
       ...
       if (current->node_parrent != NULL && current->node_parrent->node_nb != NULL) { 
        ...
        printf("tidak ada pewaris dari %s.\n", name);
        return NULL;
    }
}
...
void tampilkan_informasi(address root)
{
    ...
    printf("mate = %s\n", temp->node_mate ? temp->node_mate->info.nama : "null");
...
void delete_input(telm_familly *root) {
    ...
    do {
        printTree(root, 0);
        printf("Masukkan nama orang yang ingin dihapus beserta generasinya: ");
        scanf(" %[^\n]", nama);
        string_to_uppercase(nama);
        temp = search_handler(root, nama);
    ...
    printf("%s beserta keturunannya telah dihapus dari sejarah silsilah kerajaan\n", nama);
    printTree(root, 0);
    printf("\n\tPress any key to continue . . . ");
    getch();
}
